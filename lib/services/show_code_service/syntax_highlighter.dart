...
class DartSyntaxHighlighter extends SyntaxHighlighter {
  DartSyntaxHighlighter([SyntaxHighlighterStyle? style])
      : _style = style ?? SyntaxHighlighterStyle.darkThemeStyle() {
  ...
  late final List<_HighlightSpan> _spans;
  ...
  TextSpan format(String src) {
    ...
    if (_generateSpans()) {
      ...
      if (currentPosition != _src.length) {
        formattedText.add(TextSpan(text: _src.substring(currentPosition)));
      }
  ...
  bool _generateSpans() {
    ...
    while (!_scanner.isDone) {
      _scanner.scan(RegExp(r'\s+')); // Skip White space
      ...
      if (_scanner.scan(RegExp(r'\w+'))) {
        final String word = _scanner.lastMatch![0]!;
  ...
  bool _firstLetterIsUpperCase(String str) {
    return str.isNotEmpty && str[0] == str[0].toUpperCase();
  }
}
